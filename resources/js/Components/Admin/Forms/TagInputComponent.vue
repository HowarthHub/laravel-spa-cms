...
<template>
    <div>
        ...
        <div v-if="selectedTags.length" class="mb-2 flex flex-wrap gap-1.5">
            ...
            >
                {{ tag.name }}
                <button type="button" @click="removeTag(tag.id)" class="hover:text-indigo-900">
        ...
        <div class="relative">
            ...
            <ul
                ...
                class="absolute z-10 mt-1 w-full rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 max-h-40 overflow-y-auto"
            >
                <li
                    v-for="tag in filteredTags"
                    :key="tag.id"
                    @mousedown.prevent="addTag(tag.id)"
                    class="cursor-pointer px-3 py-1.5 text-sm text-gray-700 hover:bg-indigo-50"
                >
                    {{ tag.name }}
                </li>
            </ul>
