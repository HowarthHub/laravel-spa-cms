...
const formData = useForm({
    ...
    handle: props.form.handle,
    success_message: props.form.success_message || '',
    notification_email: props.form.notification_email || '',
...
watch(() => formData.name, (val) => {
    if (!manualHandle) {
        formData.handle = generateHandle(val);
    }
});
...
const submit = () => {
    ...
    formData.transform(() => data).put(`/admin/forms/${props.form.id}`);
};
</script>

<template>
    <Head :title="`Edit: ${props.form.name}`" />
    <AdminLayout>
        <template #breadcrumb>
        ...
        <form @submit.prevent="submit" class="space-y-6">
            <div class="flex items-center justify-between">
                <h1 class="text-xl font-semibold text-gray-900 dark:text-white">Edit Form</h1>
            </div>
                ...
                <div class="lg:col-span-2 space-y-5">
                    <div class="rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 space-y-4">
                        <div>
                            ...
                            />
                            <p v-if="formData.errors.name" class="mt-1 text-sm text-red-600">{{ formData.errors.name }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                :class="{ 'border-red-500': formData.errors.handle }"
                            />
                            <p v-if="formData.errors.handle" class="mt-1 text-sm text-red-600">{{ formData.errors.handle }}</p>
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                class="w-full rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm shadow-sm focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none dark:bg-gray-700 dark:text-gray-100"
                            />
                            <p v-if="formData.errors.success_message" class="mt-1 text-sm text-red-600">{{ formData.errors.success_message }}</p>
                        </div>
                    </div>
                    ...
                    <div class="rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
                        ...
                        <p v-if="formData.errors.fields" class="mb-3 text-sm text-red-600">{{ formData.errors.fields }}</p>

                        <div v-if="formData.fields.length === 0" class="text-center py-8 text-sm text-gray-500 dark:text-gray-400">
                            ...
                            >
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <div class="flex items-center gap-1">
                                        ...
                                        <button type="button" @click="moveField(index, 1)" :disabled="index === formData.fields.length - 1"
                                            class="rounded p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 disabled:opacity-30 disabled:cursor-not-allowed"
                                            title="Move Down">
                ...
                <div class="space-y-5">
                    <div class="rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 space-y-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                class="w-full rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm shadow-sm focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none dark:bg-gray-700 dark:text-gray-100"
                            />
                            <p v-if="formData.errors.notification_email" class="mt-1 text-sm text-red-600">{{ formData.errors.notification_email }}</p>
                        ...
                        <div class="flex items-center justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <button type="button" @click="formData.is_active = !formData.is_active"
                                :class="formData.is_active ? 'bg-cyan-600' : 'bg-gray-300 dark:bg-gray-600'"
                    ...
                    <div class="rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
                        <button type="submit" :disabled="formData.processing"
                            ...
                            {{ formData.processing ? 'Saving...' : 'Update Form' }}
                        </button>
                    </div>
                </div>
            </div>
