...
const statCards = [
    ...
    { label: 'Total Users', key: 'totalUsers', color: 'bg-indigo-500', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z', href: '/admin/users' },
];
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <Link
                    ...
                    :key="card.key"
                    :href="card.href"
                    class="overflow-hidden rounded-lg bg-white shadow hover:shadow-md transition-shadow"
                >
                    <div class="p-5">
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                <div class="overflow-hidden rounded-lg bg-white shadow">
                    <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                        ...
                        <Link href="/admin/posts" class="text-xs text-indigo-600 hover:text-indigo-500">View all</Link>
                    </div>
                    <div v-if="recentPosts.length" class="divide-y divide-gray-200">
                        <Link v-for="post in recentPosts" :key="post.id" :href="`/admin/posts/${post.id}/edit`"
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="text-xs text-gray-400">by {{ post.author?.name || 'Unknown' }}</p>
                            </div>
                            <div class="ml-4 flex items-center gap-2">
                                <span :class="statusBadge(post.status)" class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium capitalize">
                                    {{ post.status }}
                                </span>
                                <span class="text-xs text-gray-400">{{ new Date(post.updated_at).toLocaleDateString() }}</span>
                            </div>
                        </Link>
                    </div>
                    <div v-else class="px-6 py-8 text-center text-sm text-gray-500">No posts yet.</div>
                </div>
                ...
                <div class="overflow-hidden rounded-lg bg-white shadow">
                    <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                        ...
                        <Link href="/admin/enquiries" class="text-xs text-indigo-600 hover:text-indigo-500">View all</Link>
                    </div>
                    <div v-if="recentEnquiries.length" class="divide-y divide-gray-200">
                        <Link v-for="enquiry in recentEnquiries" :key="enquiry.id" :href="`/admin/enquiries/${enquiry.id}`"
                            class="flex items-center justify-between px-6 py-3 hover:bg-gray-50"
                            :class="{ 'bg-blue-50/50': enquiry.status === 'new' }">
                            <div class="min-w-0 flex-1">
                                <p class="truncate text-sm font-medium text-gray-900" :class="{ 'font-semibold': enquiry.status === 'new' }">
                                    {{ enquiry.name }}
                                </p>
                                <p class="truncate text-xs text-gray-400">{{ enquiry.subject || enquiry.email }}</p>
                            </div>
                            <div class="ml-4 flex items-center gap-2">
                                <span :class="statusBadge(enquiry.status)" class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium capitalize">
                                    {{ enquiry.status }}
                                </span>
                                <span class="text-xs text-gray-400">{{ new Date(enquiry.created_at).toLocaleDateString() }}</span>
                            </div>
                        </Link>
                    </div>
                    <div v-else class="px-6 py-8 text-center text-sm text-gray-500">No enquiries yet.</div>
                </div>

                <!-- Recent Pages -->
                <div class="overflow-hidden rounded-lg bg-white shadow lg:col-span-2">
                    <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                        <h3 class="text-sm font-medium text-gray-900">Recent Pages</h3>
                        <Link href="/admin/pages" class="text-xs text-indigo-600 hover:text-indigo-500">View all</Link>
                    </div>
                    <div v-if="recentPages.length" class="divide-y divide-gray-200">
                        <Link v-for="page in recentPages" :key="page.id" :href="`/admin/pages/${page.id}/edit`"
                            class="flex items-center justify-between px-6 py-3 hover:bg-gray-50">
                            <div class="min-w-0 flex-1">
                                <p class="truncate text-sm font-medium text-gray-900">{{ page.title }}</p>
                                <p class="text-xs text-gray-400">by {{ page.author?.name || 'Unknown' }}</p>
                            </div>
                            <div class="ml-4 flex items-center gap-2">
                                <span :class="statusBadge(page.status)" class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium capitalize">
                                    {{ page.status }}
                                </span>
                                <span class="text-xs text-gray-400">{{ new Date(page.updated_at).toLocaleDateString() }}</span>
                            </div>
                        </Link>
                    </div>
                    <div v-else class="px-6 py-8 text-center text-sm text-gray-500">No pages yet.</div>
                </div>
            </div>
        </div>
    </AdminLayout>
